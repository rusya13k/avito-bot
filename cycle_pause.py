...
# Обеденный перерыв (окно LUNCH_WINDOW) — короткий, в отличие от ужина.
# Реальный человек обедает ~30-40 мин, а не уходит на 2-5 часов. Поэтому в
# обеденном окне длительность перерыва берётся из этих границ, а не из
# DEFAULT_LONG_BREAK_*. Вечерний (DINNER_WINDOW) перерыв остаётся длинным.
DEFAULT_LUNCH_BREAK_MIN_MIN = 25.0  # ~25 мин
DEFAULT_LUNCH_BREAK_MAX_MIN = 40.0  # ~40 мин
...
def pick_cycle_pause(
    ...
    *,
    ...
    - lognormal regular pause в [pause_min, pause_max] минут (норма)
      ИЛИ
    - break в окне обеда/ужина, если не достигнут лимит long_breaks_per_day
      и выпал бросок. Длительность зависит от окна:
        • обед (LUNCH_WINDOW 12-14) — короткий [lunch_break_min, lunch_break_max]
          (~25-40 мин): человек обедает, а не пропадает на полдня;
        • ужин (DINNER_WINDOW) / вне окон — длинный [long_break_min,
          long_break_max] (~2-5 часов).
      Оба возвращаются с label "long_break" (и инкрементируют дневной счётчик).

    Вероятность break повышена в окнах обеда (12-14) и ужина (18-21).
    ...
    lunch_break_min = float(
        _resolve(account, cfg, "lunch_break_min_min", DEFAULT_LUNCH_BREAK_MIN_MIN)
    )
    lunch_break_max = float(
        _resolve(account, cfg, "lunch_break_max_min", DEFAULT_LUNCH_BREAK_MAX_MIN)
    )
    ...
    if can_long_break:
        ...
        if rng.random() < chance:
            # Обед (LUNCH_WINDOW) — короткий перерыв 25-40 мин; ужин/вне —
            # длинный 2-5ч. Так бот не «пропадает» на полдня в обеденные часы,
            # сохраняя длинный вечерний перерыв как естественный паттерн.
            if _in_window(hour, LUNCH_WINDOW):
                seconds = _uniform_seconds(lunch_break_min, lunch_break_max, rng=rng)
            else:
